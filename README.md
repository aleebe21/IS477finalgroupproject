...
The project was a joint collaboration where everyone had their own role in contributing to our final deliverable. Each of us had an integral part in contributing - for example, Affan was responsible for the overall vision and documentation of the project including the project overview, narrative of the profiling, quality check and analysis interpretation - contributing to the Status Report and Project Plan alongside Om. In addition, Affan implemented the analysis pipeline from start to finish by generating visuals and making the whole project reproducible from the final product. Om was responsible for the technical preparations of the datasets including gathering all of the data sources in their raw form, implementing the data cleaning script that served as a preprocessing tool for both of our selected EV and AFDC datasets, as well as outlining the ideas behind merging both in the cities format. The two of us worked together and met continously throughout this process to assess and analyze the end results, and to confirm that the repository, output and documentation structure met the requirements for Milestone 4 by validating that the merged dataset was in fact accurate. By allocating our tasks as such, it made it easy for us create this project and finish it with high efficiency and with a low margin of error with regard to clarity, accuracy, and reproducibility.
---

## Summary
This project seeks to understand what specific patterns of electric vehicle (EV) purchases compared to charging station availability as related to neighborhoods across the state of Washington. As the state, and nation overall, moves towards EV adoption, this research is important for equity and future patterns. This is specifically important since if certain areas have greater populations of EVs than charging stations, it will result in range anxiety, overpopulated charging stations, or even the inability to purchase an EV in the first place due to no charging stations in proximity.
Initially, the project scope and intention was to compare data at the county level; however, once transitioning from a Kaggle copy of the Department of Energy's fueling stations to the official Alternative Fuels Data Center (AFDC) export, it was discovered that the AFDC file does not possess a county level associated with each fueling station. Instead, the AFDC data includes city, zip code and lat/long. Hence why we pivoted - to ensure data quality without creating an additional crosswalk from zip to county levels (which would be unnecessarily complicated), we switched our analysis from a county to a city level. Thus, we maintain the policy intention (to find deficiencies in charging availability per EV access) but report results on a city level across Washington instead.

We integrate two of our main datasets as pre-approved by our TA:
